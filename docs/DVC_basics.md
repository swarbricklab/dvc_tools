...
The workspace is easy to understand. This consists of the files and folders in the project directory. These files can be
...
(*) Links can be hardlinks, symlinks, reflinks or plain copies. On NCI we use hardlinks when possible to reduce the inode count, and fall back to symlinks when hardlinks are not available. (Hardlinks cannot be created across different volumes, or -- on systems with `fs.protected_hardlinks` enabled, which includes NCI -- to files owned by somebody else.) The link type is determined by the `cache.type` configuration item.
...
- under the cache root, DVC 3 stores content at `files/md5/XX/YYYY...` (where `XX` is the first two characters of the checksum). DVC 2 used `XX/YYYY...` directly -- see [`dt migrate`](migrate.md) for converting a v2 project
- multiple clones can share an external cache; set the `cache.shared` configuration item to `group` so that cached files are group-writable and everyone in the project can add to it