...
- **Singularity GHCR auth requires persistent credentials, not just env vars.** `absconda deploy` calls `singularity pull docker://ghcr.io/...` without explicit `--docker-username`/`--docker-password` flags, relying instead on `SINGULARITY_DOCKER_*` env vars. In practice, NCI's singularity ignores these in favour of credentials stored in `~/.singularity/remote.yaml`. The "Refresh Singularity GHCR credentials" step in the deploy job runs `singularity remote login docker://ghcr.io` on the runner (which shares the NCI home directory with the PBS compute node) before each deploy. To rotate the PAT, update `absconda-github-token` and `absconda-github-username` in Secret Manager — no manual `singularity remote login` needed.
