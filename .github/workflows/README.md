...
- **Singularity GHCR auth requires persistent credentials, not just env vars.** `absconda deploy` calls `singularity pull docker://ghcr.io/...` without explicit `--docker-username`/`--docker-password` flags, relying instead on `SINGULARITY_DOCKER_*` env vars. In practice, NCI's singularity ignores these in favour of credentials stored in `~/.singularity/remote.yaml`. The "Refresh Singularity GHCR credentials on NCI compute node" step runs `singularity remote login docker://ghcr.io` inside a `qxub exec --mod singularity` job before each deploy, so the credentials are written from the same context that `absconda deploy` will later read from. The token is staged in a `chmod 600` file under `$HOME` (NCI shared home, visible to PBS compute nodes as the same user) and never appears on the `qsub` command line. To rotate the PAT, update `absconda-github-token` and `absconda-github-username` in Secret Manager — no manual `singularity remote login` needed.
